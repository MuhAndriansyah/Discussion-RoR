class ChannelsController < ApplicationController
  ...
  def index
    ...
    @discussions = Discussion.all.order('create_at desc')
  end

  # GET /channels/1
  # GET /channels/1.json
  def show
    @discussions = Discussion.where('channel_id = ?', @channel.id) #Akan menampilkan seluruh diskusi pada sebuah channel
    @channels = Channel.all
  end
